...
BEGIN_MESSAGE_MAP(asteroid_editor, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_FIELD_NUM, OnFieldNumChange)
...
BOOL asteroid_editor::OnCommand(WPARAM wParam, LPARAM lParam) 
{
	...
	id = LOWORD(wParam);
	if (id >= ID_FIELD_MENU && id < ID_FIELD_MENU + MAX_ASTEROID_FIELDS) {
...
int asteroid_editor::query_modified()
{
	for (int i = 0; i < MAX_ASTEROID_FIELDS; i++) {
		...
		if (current.num_initial_asteroids != compare_field->num_initial_asteroids) return 1;
		if (vm_vec_dist_quick(&current.vel, &compare_field->vel) > 0.0f) return 1;
		if (current.min_bound.xyz.x != compare_field->min_bound.xyz.x) return 1;
		if (current.min_bound.xyz.y != compare_field->min_bound.xyz.y) return 1;
		if (current.min_bound.xyz.z != compare_field->min_bound.xyz.z) return 1;
		if (current.max_bound.xyz.x != compare_field->max_bound.xyz.x) return 1;
		if (current.max_bound.xyz.y != compare_field->max_bound.xyz.y) return 1;
		if (current.max_bound.xyz.z != compare_field->max_bound.xyz.z) return 1;
		if (current.has_inner_bound != compare_field->has_inner_bound) return 1;
		if (current.field_type != compare_field->field_type) return 1;
		if (current.debris_genre != compare_field->debris_genre) return 1;
		if (current.has_inner_bound) {
			if (current.inner_max_bound.xyz.x != compare_field->inner_max_bound.xyz.x) return 1;
			if (current.inner_max_bound.xyz.y != compare_field->inner_max_bound.xyz.y) return 1;
			if (current.inner_max_bound.xyz.z != compare_field->inner_max_bound.xyz.z) return 1;
			if (current.inner_min_bound.xyz.x != compare_field->inner_min_bound.xyz.x) return 1;
			if (current.inner_min_bound.xyz.y != compare_field->inner_min_bound.xyz.y) return 1;
			if (current.inner_min_bound.xyz.z != compare_field->inner_min_bound.xyz.z) return 1;
		}
		if (current.enhanced_visibility_checks != compare_field->enhanced_visibility_checks) return 1;
		if (current.target_names != compare_field->target_names) return 1;
		if (current.field_debris_type != compare_field->field_debris_type) return 1;
		if (current.field_asteroid_type != compare_field->field_asteroid_type) return 1;
	}
	return 0;
}
...
int asteroid_editor::validate_data()
{
	if (!m_enable_asteroids) {
		...
		// check x
		if (a_field[cur_field].max_bound.xyz.x < a_field[cur_field].min_bound.xyz.x) {
			MessageBox("Asteroid x min is greater than max");
			return 0;
		}

		// check y
		if (a_field[cur_field].max_bound.xyz.y < a_field[cur_field].min_bound.xyz.y) {
			MessageBox("Asteroid y min is greater than max");
			return 0;
		}

		// check z
		if (a_field[cur_field].max_bound.xyz.z < a_field[cur_field].min_bound.xyz.z) {
			MessageBox("Asteroid z min is greater than max");
			return 0;
		}

		// check if inner bounds enabled
		if (a_field[cur_field].has_inner_bound) {
			if (a_field[cur_field].inner_max_bound.xyz.x < a_field[cur_field].inner_min_bound.xyz.x) {
				MessageBox("Asteroid x inner min is greater than inner max");
				return 0;
			}

			if (a_field[cur_field].inner_max_bound.xyz.y < a_field[cur_field].inner_min_bound.xyz.y) {
				MessageBox("Asteroid y inner min is greater than inner max");
				return 0;
			}

			if (a_field[cur_field].inner_max_bound.xyz.z < a_field[cur_field].inner_min_bound.xyz.z) {
				MessageBox("Asteroid z inner min is greater than inner max");
				return 0;
			}

			// check x thickness
			if (a_field[cur_field].inner_min_bound.xyz.x - MIN_BOX_THICKNESS < a_field[cur_field].min_bound.xyz.x) {
				MessageBox("Asteroid x thickness from outer box to inner box must be > 400");
				return 0;
			}

			if (a_field[cur_field].inner_max_bound.xyz.x + MIN_BOX_THICKNESS > a_field[cur_field].max_bound.xyz.x) {
				MessageBox("Asteroid x thickness from outer box to inner box must be > 400");
				return 0;
			}

			// check y thickness
			if (a_field[cur_field].inner_min_bound.xyz.y - MIN_BOX_THICKNESS < a_field[cur_field].min_bound.xyz.y) {
				MessageBox("Asteroid y thickness from outer box to inner box must be > 400");
				return 0;
			}

			if (a_field[cur_field].inner_max_bound.xyz.y + MIN_BOX_THICKNESS > a_field[cur_field].max_bound.xyz.y) {
				MessageBox("Asteroid y thickness from outer box to inner box must be > 400");
				return 0;
			}

			// check z thickness
			if (a_field[cur_field].inner_min_bound.xyz.z - MIN_BOX_THICKNESS < a_field[cur_field].min_bound.xyz.z) {
				MessageBox("Asteroid z thickness from outer box to inner box must be > 400");
				return 0;
			}

			if (a_field[cur_field].inner_max_bound.xyz.z + MIN_BOX_THICKNESS > a_field[cur_field].max_bound.xyz.z) {
				MessageBox("Asteroid z thickness from outer box to inner box must be > 400");
				return 0;
			}
		}
		...
		if (a_field[cur_field].field_type == FT_PASSIVE) {
			if (a_field[cur_field].debris_genre == DG_DEBRIS) {
				if (a_field[cur_field].field_debris_type.size() == 0) {
		...
		if (a_field[cur_field].debris_genre == DG_ASTEROID) {
			if (a_field[cur_field].field_asteroid_type.size() == 0) {
...
void asteroid_editor::OnOK()
{
	...
	for (int i = 0; i < MAX_ASTEROID_FIELDS; i++) {
		if (a_field[i].num_initial_asteroids > 0) {
			if (a_field[i].debris_genre == DG_ASTEROID) {
				a_field[i].field_debris_type.clear();
			} else {
				a_field[i].field_asteroid_type.clear();
			}
			Asteroid_fields.push_back(a_field[i]);
		}
	}
...
void asteroid_editor::OnEnableAsteroids() 
{
	...
	GetDlgItem(IDC_SELECT_DEBRIS)->EnableWindow(m_enable_asteroids && (a_field[cur_field].debris_genre == DG_DEBRIS));
	GetDlgItem(IDC_SELECT_ASTEROID)->EnableWindow(m_enable_asteroids && (a_field[cur_field].debris_genre == DG_ASTEROID));
	...
	auto* field_combo = (CComboBox*)GetDlgItem(IDC_FIELD_NUM);
	field_combo->ResetContent();
	...
	GetDlgItem(IDC_FIELD_NUM)->EnableWindow(num_fields > 0);
	GetDlgItem(IDC_ADD_FIELD)->EnableWindow(num_fields < MAX_ASTEROID_FIELDS);
	GetDlgItem(IDC_REMOVE_FIELD)->EnableWindow(num_fields > 0);
...
void asteroid_editor::OnAddField()
{
	...
	for (int i = 0; i < MAX_ASTEROID_FIELDS; i++) {
		if (a_field[i].num_initial_asteroids <= 0) {
			a_field[i] = {};
			a_field[i].num_initial_asteroids = 10;
			cur_field = i;
...
{
	update_init();
	if (a_field[cur_field].num_initial_asteroids <= 0) {
		return;
	}

	for (int i = cur_field; i < MAX_ASTEROID_FIELDS - 1; i++) {
		a_field[i] = a_field[i + 1];
	}
	a_field[MAX_ASTEROID_FIELDS - 1] = {};

	if (cur_field >= MAX_ASTEROID_FIELDS - 1 || a_field[cur_field].num_initial_asteroids <= 0) {
		cur_field = MAX(cur_field - 1, 0);
	}
	update_init();
}

void asteroid_editor::OnFieldNumChange()
{
	auto* combo = (CComboBox*)GetDlgItem(IDC_FIELD_NUM);
