...
void sexp_tree::right_clicked(int mode)
{
	...
	if (action_ptr->enabled && !action_ptr->children.empty()) {
		item_flags = MF_POPUP;
		appendActionsToMenu(replace_var_submenu, action_ptr->children, cmd_action_next);
	} else {
	...
	if (action_ptr->enabled && !action_ptr->children.empty()) {
		item_flags = MF_POPUP;
		appendActionsToMenu(replace_container_name_submenu, action_ptr->children, cmd_action_next);
	} else {
	...
	if (action_ptr->enabled && !action_ptr->children.empty()) {
		item_flags = MF_POPUP;
		appendActionsToMenu(replace_container_data_submenu, action_ptr->children, cmd_action_next);
	} else {
...
BOOL sexp_tree::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	// Command IDs from ID_SEXP_ACTION_BASE are assigned in right_clicked() and mapped
	// to their leaf SexpContextAction. The model's executeAction() does all the work;
	// we just rebuild the tree UI from the updated model.
	if (id >= ID_SEXP_ACTION_BASE) {
		...
		if (it_action != m_pending_context_actions.end()) {
			...
			if (m_model->executeAction(item_index, leaf.id, &leaf.param)) {
				build_tree();
				*modified = 1;
			}
			return 1;
