	  ...
	  _viewport(viewport), _fredView(parent)
{
	...
	// Undo/redo rewrites mission data without going through the model, so refresh on
	// any mission change as well -- this is what makes an undone edit visually revert.
	//
	// This must re-read the model rather than just repaint: the model caches the
	// arrival/departure formula indices, and a command like SexpCueEditCommand frees
	// the old sexp chain before emitting missionChanged. Repainting off the stale
	// cache would walk freed nodes. initializeData() emits shipMarkingChanged, which
	// is already wired to initializeUi()/updateUi() above, so it repaints for us.
