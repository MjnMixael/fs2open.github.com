...
void BriefingMapWidget::mousePressEvent(QMouseEvent* event) {
	...
	Q_EMIT iconDragStarted(anchorIndex);
}
...
void BriefingMapWidget::mouseReleaseEvent(QMouseEvent* event) {
	...
	// Resolve a drag-box selection (or a plain click on empty space).  A box select never sets
	// the icon-drag state (mousePressEvent clears it on the empty-space path), so returning
	// here cannot strand a drag or skip the iconDragFinished below.
