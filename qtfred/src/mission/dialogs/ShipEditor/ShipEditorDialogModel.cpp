...
std::vector<std::pair<SCP_string, Qt::CheckState>> ShipEditorDialogModel::getAcceptedOrders()
{
	std::vector<std::pair<SCP_string, Qt::CheckState>> acceptedOrders;
	...
	for (size_t order_id : default_orders) {
		...
		if (!selected_ships.empty() && accepted_count == static_cast<int>(selected_ships.size())) {
			state = Qt::Checked;
...
void ShipEditorDialogModel::setAcceptedOrders(const std::vector<std::pair<SCP_string, Qt::CheckState>>& newOrders)
{
	orders = newOrders;
	...
	for (auto* ptr = GET_FIRST(&obj_used_list); ptr != END_OF_LIST(&obj_used_list); ptr = GET_NEXT(ptr)) {
		if (((ptr->type == OBJ_SHIP) || (ptr->type == OBJ_START)) && (ptr->flags[Object::Object_Flags::Marked])) {
			...
			SCP_set<size_t> new_orders_set = Ships[i].orders_accepted;
			for (size_t list_index = 0; list_index < orders.size() && list_index < accepted_order_ids.size(); ++list_index) {
				...
				if (!default_orders.contains(order_id)) {
					continue;
				}

				switch (orders[list_index].second) {
				case Qt::Checked:
					new_orders_set.insert(order_id);
					break;
				case Qt::Unchecked:
					new_orders_set.erase(order_id);
					break;
				case Qt::PartiallyChecked:
					// Preserve per-ship value in mixed state.
					break;
				}
			}
