	...
	if ( wl_icon_being_carried() ) {
		...
		if ( Wl_icons[Carried_wl_icon.weapon_class].can_use_for_ship )
		{
			...
			if(icon->icon_bmaps[WEAPON_ICON_FRAME_SELECTED] != -1)
			{
				...
				{
					//Draw the model
					draw_model_icon(icon->model_index, MR_NO_FOGGING | MR_NO_LIGHTING, sx, sy, w, h, nullptr, wip, 1.0f / 2.5f, GR_RESIZE_MENU);
				}
...
void wl_render_icon(int index, int x, int y, int num, int draw_num_flag, int hot_mask, int hot_bank_mask, int select_mask)
{
	...
	else
	{
		...
		{
			//Draw the model
			draw_model_icon(icon->model_index, MR_NO_FOGGING | MR_NO_LIGHTING, x, y, 56, 24, nullptr, &Weapon_info[index], 1.0f / 2.5f, GR_RESIZE_MENU);
		}
		else if(icon->laser_bmap != -1)
		{
