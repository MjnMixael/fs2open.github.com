...
void ship_select_do(float frametime)
{
	...
	if ( ss_icon_being_carried() ) {
		...
		else
		{
			...
			if(Ss_icons[Carried_ss_icon.ship_class].model_index != -1)
			{
				draw_model_icon(Ss_icons[Carried_ss_icon.ship_class].model_index, MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_LIGHTING, sx, sy, w, h, sip, nullptr, 1.0f / 1.25f, GR_RESIZE_MENU);
			}
		}
	}
...
void draw_ship_icon_with_number(int screen_offset, int ship_class)
{
	...
	else
	{
		...
		if(ss_icon->model_index != -1)
		{
			draw_model_icon(ss_icon->model_index, MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_LIGHTING, Ship_list_coords[gr_screen.res][screen_offset][0],Ship_list_coords[gr_screen.res][screen_offset][1], 32, 28, sip, nullptr, 1.0f / 1.25f, GR_RESIZE_MENU);
		}
	}
...
void draw_wing_block(int wb_num, int hot_slot, int selected_slot, int class_select, bool ship_selection, bool always_highlight_ply )
{
	...
	for ( i = 0; i < MAX_WING_SLOTS; i++ ) {
		...
		else if(color_to_draw != NULL)
		{
			...
			draw_model_icon(icon->model_index, MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_LIGHTING, Wing_icon_coords[gr_screen.res][slot_index][0], Wing_icon_coords[gr_screen.res][slot_index][1], 32, 28, sip, nullptr, 1.0f / 1.25f, GR_RESIZE_MENU);
		}
	}
}
	...
	else
	{
		...
		else
		{
			...
			if(icon->model_index != -1)
			{
				...
				draw_model_icon(icon->model_index, MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_LIGHTING, x, y, 32, 28, sip, nullptr, 1.0f / 1.25f, GR_RESIZE_MENU);
			}
		}
	}
