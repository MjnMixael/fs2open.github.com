...
int parse_weapon(int subtype, bool replace, const char *filename)
{
	...
	if (optional_string("$Tech Model:")) {
		...
		if (optional_string("+Icon_closeup_pos:")) {
			...
			wip->icon_closeup_pos_set = true;
		}

		if (optional_string("+Icon_closeup_zoom:")) {
			stuff_float(&wip->icon_closeup_zoom);
			wip->icon_closeup_zoom_set = wip->icon_closeup_zoom > 0.0f;
			if (!wip->icon_closeup_zoom_set) {
				mprintf(("Warning!  Weapon '%s' has a +Icon_closeup_zoom value that is less than or equal to 0 (%f). Ignoring icon override.\n", wip->name, wip->icon_closeup_zoom));
				wip->icon_closeup_zoom = 0.0f;
			}
		}
	}
...
void weapon_info::reset()
{
	...
	vm_vec_zero(&this->icon_closeup_pos);
	this->icon_closeup_zoom = 0.0f;
	this->icon_closeup_pos_set = false;
	this->icon_closeup_zoom_set = false;
