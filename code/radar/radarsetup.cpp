...
void HudGaugeRadar::drawRange()
{
	...
	case RR_SHORT:
		renderPrintf(position[0] + Radar_dist_offsets[RR_SHORT][0], position[1] + Radar_dist_offsets[RR_SHORT][1], 1.0, false, "%s", XSTR( "2k", 467));
		break;

	case RR_LONG:
		renderPrintf(position[0] + Radar_dist_offsets[RR_LONG][0], position[1] + Radar_dist_offsets[RR_LONG][1], 1.0, false, "%s", XSTR( "10k", 468));
		break;
	...
	case RR_INFINITY:
		if (Unicode_text_mode) {
			...
			renderPrintf(position[0] + Radar_dist_offsets[RR_INFINITY][0], position[1] + Radar_dist_offsets[RR_INFINITY][1], 1.0, false, "\xE2\x88\x9E");
		} else {
			renderPrintf(position[0] + Radar_dist_offsets[RR_INFINITY][0], position[1] + Radar_dist_offsets[RR_INFINITY][1], 1.0, false, "%c", Radar_infinity_icon);
		}
		break;
