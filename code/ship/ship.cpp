...
static void parse_ship_values(ship_info* sip, const bool is_template, const bool first_time, const bool replace)
{
	...
	if (optional_string("$Icon_closeup_zoom:")) {
		...
		if (sip->icon_closeup_zoom <= 0.0f) {
			mprintf(("Warning!  Ship '%s' has a $Icon_closeup_zoom value that is less than or equal to 0 (%f). Ignoring override.\n", sip->name, sip->icon_closeup_zoom));
			sip->icon_closeup_zoom = -1.0f;
		}
	}
