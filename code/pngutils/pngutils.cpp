...
static inline bool parse_fso_keyframe_itxt(const SCP_vector<ubyte>& chunk_data, int& out_keyframe)
{
	...
	const auto* data_end = reinterpret_cast<const char*>(&chunk_data[chunk_data.size() - 4]);
...
void apng_ani::_process_chunk()
{
	...
	if (_id == id_acTL && !_got_IDAT && !_got_acTL) {
		...
		if (parse_fso_keyframe_itxt(_chunk.data, parsed_keyframe)) {
