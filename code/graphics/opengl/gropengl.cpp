...
void gr_opengl_print_screen(const char *filename)
{
	...
	// Screenshots are requested during input processing which can happen before all overlays are rendered for the current
	// frame (e.g. ImGui in the lab). In window-res mode the in-progress frame is rendered to Back_framebuffer, so reading
	// from that can miss late overlays. Read the currently displayed image from the default framebuffer instead.
