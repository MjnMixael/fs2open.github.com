...
void HudGaugeEscort::render(float  /*frametime*/, bool config)
{
	...
	renderString(x + static_cast<int>(header_text_offsets[0] * scale), y + static_cast<int>(header_text_offsets[1] * scale), header_text, scale, config);

	int lx = x + static_cast<int>(list_start_offsets[0] * scale);
	int ly = y + static_cast<int>(list_start_offsets[1] * scale);
	...
	if(num_escort_ships > 0)
	{
		for(; i < num_escort_ships; i++)
		{
			if(i != 0)
			{
				lx += static_cast<int>(entry_stagger_w * scale);
				ly += static_cast<int>(entry_h * scale);
			}
	...
	renderBitmap(Escort_gauges[2].first_frame, lx, ly + static_cast<int>(bottom_bg_offset * scale), scale, config);
	renderIcon(lx, ly, i, scale, config);
	...
	if (config) {
		...
		hud_config_set_mouse_coords(gauge_config, x, lx + static_cast<int>(bmw * scale), y, ly + static_cast<int>(bmh * scale));
	}
}
...
void HudGaugeEscort::renderIcon(int x, int y, int index, float scale, bool config)
{
	...
	if ((config && index == 1) || (!config && (( (sp->flags[Ship::Ship_Flags::Disabled]) || (ship_subsys_disrupted(sp, SUBSYSTEM_ENGINE)) )))) {		
		renderString( x + static_cast<int>(ship_status_offsets[0] * scale), y + static_cast<int>(ship_status_offsets[1] * scale), EG_NULL, XSTR( "D", 284), scale, config);				
	}
	...
	const int w = font::force_fit_string(buf, 255, static_cast<int>(ship_name_max_width * scale), scale);
	
	if (right_align_names) {
		renderString( x + static_cast<int>(ship_name_offsets[0] * scale) + static_cast<int>(ship_name_max_width * scale) - w, y + static_cast<int>(ship_name_offsets[1] * scale), EG_ESCORT1 + index, buf, scale, config);
	} else {
		renderString( x + static_cast<int>(ship_name_offsets[0] * scale), y + static_cast<int>(ship_name_offsets[1] * scale), EG_ESCORT1 + index, buf, scale, config);
	}
	...
	renderPrintfWithGauge( x+static_cast<int>(ship_integrity_offsets[0] * scale) + offset, y+static_cast<int>(ship_integrity_offsets[1] * scale), EG_NULL, scale, config, "%d", screen_integrity);
