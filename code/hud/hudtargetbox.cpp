...
void HudGaugeTargetBox::render(float frametime, bool config)
{
	...
	if(config || Target_display_cargo) {
		...
		renderString(x + static_cast<int>(Cargo_string_offsets[0] * scale), y + static_cast<int>(Cargo_string_offsets[1] * scale), EG_TBOX_CARGO, config ? "cargo: <unknown>" : Cargo_string, scale, config);
	}
}
...
void HudGaugeTargetBox::renderTargetIntegrity(int disabled,int force_obj_num, bool config)
{
	...
	if (!config && disabled ) {
		renderBitmap(Integrity_bar.first_frame, x + static_cast<int>(Integrity_bar_offsets[0] * scale), y + static_cast<int>(Integrity_bar_offsets[1] * scale));
		return;
	}
	...
	renderString(x + static_cast<int>(Status_offsets[0] * scale), y + static_cast<int>(Status_offsets[1] * scale), EG_TBOX_INTEG, buf, scale, config);	
...
void HudGaugeTargetBox::renderTargetShipInfo(object *target_objp, bool config)
{
	...
	renderString(x + static_cast<int>(Name_offsets[0] * scale), y + static_cast<int>(Name_offsets[1] * scale), EG_TBOX_NAME, outstr_name, scale, config);	
	renderString(x + static_cast<int>(Class_offsets[0] * scale), y + static_cast<int>(Class_offsets[1] * scale), EG_TBOX_CLASS, outstr_class, scale, config);
	...
	renderPrintfWithGauge(x + static_cast<int>(Hull_offsets[0] * scale)-w, y + static_cast<int>(Hull_offsets[1] * scale), EG_TBOX_HULL, scale, true, "%s", outstr);	
	setGaugeColor(HUD_C_NONE, config);
...
void HudGaugeTargetBox::showTargetData(float  /*frametime*/, bool config)
{
	...
	renderString(x + static_cast<int>(Dist_offsets[0] * scale), y + static_cast<int>(Dist_offsets[1] * scale), EG_TBOX_DIST, outstr, scale, config);	
