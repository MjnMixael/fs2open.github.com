...
void HudGaugeSquadMessage::render(float  /*frametime*/, bool config)
{
	...
	for ( i = 0; i < nitems; i++ ) {
		...
		renderPrintfWithGauge(sx, sy, EG_SQ1 + i, config, NOX("%1d."), item_num);
	...
	if ( First_menu_item > 0 ) {
		renderPrintf(position[0] + Pgup_offsets[0], position[1] + Pgup_offsets[1], config, "%s", XSTR( "[pgup]", 312) );
	}

	if ( (First_menu_item + nitems) < Num_menu_items ) {
		renderPrintf(position[0] + Pgdn_offsets[0], position[1] + Pgdn_offsets[1], config, "%s", XSTR( "[pgdn]", 313));
	}

	if ( messaging_allowed ) {
		if ( none_valid ){
			renderPrintf( sx, by - Item_h + 2, config, "%s", XSTR( "No valid items", 314));
		} else if (Msg_shortcut_command != -1){
			renderPrintf( sx, by - Item_h + 2, config, "%s", comm_order_get_text(Msg_shortcut_command));
		}
	} else {
		// if this player is not allowed to message, then display message saying so
		renderPrintf( sx, by - Item_h + 2, config, "%s", XSTR( "Not allowed to message", 315));
	}
